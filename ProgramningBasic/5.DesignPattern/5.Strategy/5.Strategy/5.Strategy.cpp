...
class Hill : public Effect
{
	...
	{
		target.m_sStatus.nHP += caster.m_sStatus.nInt; 
	}
};
...
public:
	void Use(Player& caster, Player& target)
	{
		target.m_sStatus.nHP += caster.m_sStatus.nStr * fRat;
	}
};
...
public:
	void Ative(Player& caster, Player& target)
	{
		...
		caster.m_sStatus.nHP += target.m_sStatus.nStr;
	}
};

class EffectSlash : public Skill
{
	void Ative(Player& caster, Player& target)
	{
		target.m_sStatus.nHP = caster.m_sStatus.nStr;
	}
};
...
public:
	...
	void Init()
	{
		...
		m_cItemManager;
		m_cItemManager.SaveFile();
		m_cItemManager.LoadFile();
	}
